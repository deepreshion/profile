...
const drawShape = (shape: HeroScene['shapes'][number], time: number) => {
  ...
  const wobble = Math.sin(time * .0011 + shape.phase) * 4
  context.save()
  context.translate(shape.x + pointerX * .012, shape.y + pointerY * .012 + scrollOffset * .04)
  context.rotate(shape.rotation + time * .00004 + wobble * .002)
  ...
  if (shape.sides === 4) {
    ...
    context.strokeStyle = `rgba(255,255,255,${shape.opacity * .45})`
    context.stroke()
  }
  context.restore()
}
...
const drawScene = (time = 0) => {
  ...
  const scrollPhase = scrollOffset * .0015
  pointerX += (targetPointerX - pointerX) * .06 * motion
  pointerY += (targetPointerY - pointerY) * .06 * motion

  context.clearRect(0, 0, width, height)

  const baseGradient = context.createRadialGradient(width * .64, height * .36, 0, width * .64, height * .36, width * .72)
  baseGradient.addColorStop(0, 'rgba(183,216,75,.095)')
  baseGradient.addColorStop(.36, 'rgba(65,78,28,.07)')
  baseGradient.addColorStop(1, 'rgba(17,17,17,0)')
  context.fillStyle = baseGradient
  context.fillRect(0, 0, width, height)

  context.save()
  context.translate(pointerX * .018, pointerY * .018 + scrollOffset * .055)
  scene.terrain.lines.forEach((line) => {
    const from = scene!.terrain.points[line.from]
    const to = scene!.terrain.points[line.to]
    const pulse = .55 + Math.sin(time * .001 + from.delay + scrollPhase) * .25 * motion
  ...
  scene.terrain.points.forEach((point) => {
    const pulse = .75 + Math.sin(time * .0014 + point.delay + scrollPhase) * .25 * motion
    context!.fillStyle = `rgba(183,216,75,${(.1 + point.depth * .28) * point.intensity * pulse})`
    context!.fillRect(point.x - .75, point.y - .75, 1.5, 1.5)
  })
  context.restore()
  ...
  scene.edges.forEach((edge) => {
    ...
    const pulse = .68 + Math.sin(time * .0012 + from.phase + scrollPhase) * .32 * motion
    context!.strokeStyle = `rgba(230,230,230,${edge.opacity * pulse})`
    context!.lineWidth = 1
    context!.beginPath()
    context!.moveTo(from.x + pointerX * .012 * from.depth, from.y + pointerY * .012 * from.depth + scrollOffset * .025)
    context!.lineTo(to.x + pointerX * .012 * to.depth, to.y + pointerY * .012 * to.depth + scrollOffset * .025)
    context!.stroke()
  })
  ...
  scene.nodes.forEach((node) => {
    ...
    const yWrap = (node.y + pointerY * .018 * node.depth - dy * .018 * attraction + scrollOffset * .035) % (height + 80)
    const x = node.x + pointerX * .018 * node.depth - dx * .018 * attraction
    const y = yWrap < -40 ? yWrap + height + 80 : yWrap - 40
    const pulse = .75 + Math.sin(time * .0015 + node.phase + scrollPhase) * .25 * motion
    context!.fillStyle = node.accent
      ? `rgba(183,216,75,${(.35 + attraction * .32) * node.intensity * pulse})`
      : `rgba(238,238,238,${(.18 + attraction * .24) * node.intensity * pulse})`
    context!.beginPath()
  ...
  scene.glyphs.forEach((glyph) => {
    const drift = Math.sin(time * .00055 + glyph.phase + scrollPhase) * 6 * motion
    const x = glyph.x + pointerX * .01
    const y = ((glyph.y + drift + pointerY * .006 + scrollOffset * .045) % (height + 110)) - 55
    glyph.lines.forEach((line, index) => {
      context!.fillStyle = index === glyph.accentLine
        ? `rgba(183,216,75,${glyph.opacity + .1})`
        : `rgba(214,214,214,${glyph.opacity * .85})`
  ...
  shade.addColorStop(.28, 'rgba(17,17,17,.44)')
  shade.addColorStop(.58, 'rgba(17,17,17,.18)')
  ...
  verticalShade.addColorStop(.5, 'rgba(17,17,17,.08)')
...
const render = (time: number) => {
  ...
  if (!reducedMotion.value && props.intensity === 'full') animationFrame = requestAnimationFrame(render)
}
